...
The additive Cape Coral event-stream adapter live-verifies Code, Utility Lien, Permit, and watchlist Payoff schemas. Code/Lien/Permit watermarks and privacy-minimized snapshot items now persist in production Postgres and run daily through authenticated Vercel Cron; two consecutive production runs proved baseline suppression and no false changes. Inspections and 311 remain review-required. See [`CAPE_CORAL_EVENT_STREAM.md`](CAPE_CORAL_EVENT_STREAM.md).
