...
| P0 | Does the owner approve pushing, deploying, and publishing the prepared competition post? | the local branch is ahead of public `main`; deployment, social post, and DM are consequential external actions | owner explicitly authorizes the named actions and account/channel; execute release preflight before cutover |
...
| P2 | Which Google Search Console, Bing Webmaster, and analytics accounts should own `acrebrief.com`? | verification records and telemetry require durable business ownership | owner selects accounts; then verify domain, submit sitemap, and configure privacy-reviewed analytics |
